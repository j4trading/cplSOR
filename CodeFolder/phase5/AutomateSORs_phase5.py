...
def iterateThroughFiles():
    ...
    for i in range(0,len(filesTemplate)):
        if testMode != 0:
            ...
            filesTemplate[i][outputNameCol] = os.path.join(outputPath,filesTemplate[i][outputNameCol])
...
def processAndSaveFile(inputPathAndFile, passedIndex):
    ...
    if filesTemplate[passedIndex][actionCol] == "ytd":
        ...
        functionReturn = extractStorage()
    elif filesTemplatte[passedIndex][actionCol] == "excelStorage":
        functionReturn = excelStorage()
